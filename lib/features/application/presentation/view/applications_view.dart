...
class _ApplicationsViewState extends ConsumerState<ApplicationsView> {
  ...
  void initState() {
    super.initState();
    // Fetch the applications when the view is initialized
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(applicationViewModelProvider.notifier).getAppliedApplications();
    });
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.atEdge &&
        ...
        ref
            ...
            .getAppliedApplications(loadMore: true);
      }
    });
  }
    ...
    return NotificationListener<ScrollEndNotification>(
      ...
      child: Scaffold(
        ...
        body: RefreshIndicator(
          onRefresh: () async {
            // ref.read(applicationViewModelProvider.notifier).resetState();
          },
          child: Column(
            children: [
              if (applicationState.isLoading &&
                  applicationState.appliedApplications.isEmpty)
                const Expanded(
              ...
              else if (applicationState.appliedApplications.isEmpty)
                const Expanded(
                  child: Center(
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemCount: applicationState.appliedApplications.length +
                        (applicationState.isLoading ? 1 : 0),
                    itemBuilder: (context, index) {
                      if (index ==
                          applicationState.appliedApplications.length) {
                        return const Center(
                          child: CircularProgressIndicator(),
                        );
                      }
                      final application =
                          applicationState.appliedApplications[index];
